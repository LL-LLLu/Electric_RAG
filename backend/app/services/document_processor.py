...
class DocumentProcessor:
    ...
    def _process_single_page(self, db: Session, page_data: dict, document_id: int, original_filename: str, page_count: int, all_equipment: dict, all_relationships: list, all_ai_analyses: list) -> None:
        document = db.query(Document).filter(Document.id == document_id).first()
        if not document:
             logger.error(f"Document {document_id} not found during page processing")
             return
        ...
        logger.info(f"[PIPELINE] Page {page_num}/{page_count} | {len(equipment_list)} equipment | embed:{embed_time:.2f}s | total:{time.time()-page_start:.2f}s")
        ...
        db.commit()
        logger.info(f"Document {document_id}: Processed page {page_num}/{page_count}")
